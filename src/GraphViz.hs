...
class GraphVizable a where
  ...
  makeDot (n,e) =
    let uV = uidForVerts 1 n in
    let uE = uidForEdges uV e in
    let nodes = map (\(u,x) -> u ++ " " ++ foldPairsIntoProps (node x)) uV in
    let edges = map (\((u,a),(u',b)) -> u ++ " -> " ++ u' ++ " " ++ foldPairsIntoProps (edge a b)) uE in
    "strict digraph {\n" ++ join ";\n" nodes ++ ";\n" ++ join ";\n" edges ++ "\n}"
